...
\section{Introduction}
  ...
  \todo[inline]{It preserves so much more, but this is sort of a can of worms if all we want is a fast ArXiV paper at this point.  In particular, it
  preserves a discretized notion of ``moments''}.
...
\begin{figure}[p]
  \centering
  \includegraphics[width=0.43\textwidth]{./images/S1Wavelet_sf4nBases88scale1T1.pdf}
  \includegraphics[width=0.43\textwidth]{./images/S1Wavelet_sf4nBases88scale1T2.pdf}
  \includegraphics[width=0.43\textwidth]{./images/S1Wavelet_sf4nBases160scale2T1.pdf}
  \includegraphics[width=0.43\textwidth]{./images/S1Wavelet_sf4nBases160scale2T2.pdf}
  \includegraphics[width=0.43\textwidth]{./images/S1Wavelet_sf6nBases100scale1T1.pdf}
  \includegraphics[width=0.43\textwidth]{./images/S1Wavelet_sf6nBases100scale1T2.pdf}
  \includegraphics[width=0.43\textwidth]{./images/S1Wavelet_sf6nBases176scale2T1.pdf}
  \includegraphics[width=0.43\textwidth]{./images/S1Wavelet_sf6nBases176scale2T2.pdf}
  \caption{Ground truth densities (black) and computed densities using our method (red) with different Daubechies wavelets (rows $1$ and $2$ use the Daubehcies $4$ wavelet and rows $3$ and $4$ use the Daubechies $6$ wavelet) at different scales (rows $1$ and $3$ are computed using bases functions with a maximum resolution of $2^-1$ and rows $2$ and $4$ are computed using bases functions with a maximum resolution of $2^-2$) for the vector field $\dot{x}=\sin(2x)$ at $t=1$ (left column) and $t=2$ (right column) and with a uniform density at $t=0$. Beginning from the top to the bottom row the number of bases functions used to compute the densities depicted in each figure are $88,160,100,$ and $176$.}
