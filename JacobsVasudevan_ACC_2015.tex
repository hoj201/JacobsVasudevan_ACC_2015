...
\subsection{Background material}
  ...
  $\Phi_X^t$ at the point $x \in \R^n$. \ram{it may be clearer to say $(D\Phi_X^{-t})$ instead of $(D\Phi_X^t)^{-1}$ and similarly for $\Phi$ in the previous equation. Also we probably need a citation for the previous equation.}
...
\label{sec:naive}
  ...
  is non-negative. \ram{its not clear here why this property is important. We probably don't need to delve into this for the arxiv version but its something we will need to discuss}
  \hoj{I tried to address this briefly in the last sentence of this paragraph and the last paragraph of this section.}
...
\subsection{Densities}
  ...
  \begin{prop}
    ...
    action of $\GL(n)$ on each fiber of $\Fr M$. \ram{we may want to be clear here what ``natural'' means...}
    \hoj{meh...  Maybe we can just delete the word ``natural''.  It's hard to explain efficiently, and there is little gained from it's use, even for a mathematician.}
  \end{prop}
  ...
  given by 
  ``$(e_1,\dots,e_n) \mapsto (T\Phi \cdot e_1, \dots, T\Phi \cdot e_n)$''. \ram{this is unclear, what is $\phi$ versus $\Phi$ and what is $T$?}
  \hoj{We state that $T$ is the tangent functor in the section on notation.  We can include a definition of the Tangent functor if you like.}
...
\begin{thm}
  ...
  subring of functions on $V$. \ram{This point isn't obvious enough to me. I think we are making three separate points. First, we are saying we can represent the evolution in $V$ using Euclidean methods, we can then use Theorem 2.8 to define the density on $U$. Second, we can define a unique extension to any function or density defined on a dense set of points on any Hausdorff space (this needs a citation and is it obvious that it extends to densities?). Finally, we want to say that for many (all?) manifolds such a dense chart exists and we can construct it explicitly for many examples like we do below. Am I right?}
  \hoj{We are interpreting the use of Theorem 2.8 differently.  Here is my interpretation.
  One may think computations on $M$ are difficult, because $M$ is non-Euclidean.
  This makes the space $C^k(M), \Dens^\alpha(M), \mathfrak{X}(M)$ somewhat 
  harder to describe than the space $C^k(V), \Dens^\alpha(V), \mathfrak{X}(V)$
  where $V$ is something like a unit ball in $\mathbb{R}^n$.
  However, given a dense chart, $\varphi:U \to V$,
  a calculation involving functions, densities, and vector fields on $M$
  may be written as a calculation involving functions, densities, and vector-fields on $V$.
  By Theorem 2.8, this is done without any loss of information (because $f \mapsto \varphi_* f$ is injective).
  Now that we have a calculation which lives on $V$ we can use wavelets.
  If you want to recover the original thing on $M$ you need to invert this map, which you can do if you
  don't screw anything up because it is an injective map.}
...
\begin{align*}
  &p(x,t) = \\
  &\quad p_0\left[ \arccot \left( \cot(x)e^{-2t} \right)\right]
...
\begin{align*}
  &\psi(x,t) = \\
  &\quad \psi_0\left[\arccot\left( \cot(x)e^{-2t} \right) \right]
